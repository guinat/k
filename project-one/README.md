...
7. [LangGraph graph](docs/architecture.md): how the LLM layer is structured as a LangGraph graph: state, nodes, routing, and the wiring. Diagram (PNG + SVG + Mermaid source) in [docs/diagrams/](docs/diagrams).

## Next steps (out of scope for this set)

Machine-validatable JSON Schema file, per-OS reference standards / golden images, OS-by-OS field-applicability matrix, field-by-field rationale + Ansible collection roles.
