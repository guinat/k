...
A reminder on the data model these goals rely on: capacities are always **exact bytes** (integers, never percentages or human-readable units); a single envelope file is produced per collection run; the per-host document is OS-agnostic; and derived values — `storage_class`, all percentages, growth rates, deviations, severities, and confidence — are computed downstream by the pipeline, not collected.
...
| B-12 | Operational Efficiency | Increase analyst trust and throughput by routing only low-confidence or high-impact findings to account teams for human validation (HITL), while auto-confirming high-confidence deviations, minimizing manual review effort. |
...
### B-12 — Confidence-based validation routing

- **Business value:** auto-confirming high-confidence deviations and escalating only low-confidence or high-impact ones to account teams concentrates scarce human review (HITL) where it adds value, raising trust in the system's output.
- **Mapped objectives:** O1 (validation of low-confidence findings), O4 (operational efficiency).
- **KPI:** share of findings auto-confirmed versus escalated; false-positive rate on auto-confirmed findings; validation turnaround time.
