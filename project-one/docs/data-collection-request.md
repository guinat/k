...
- **RHEL / Oracle Linux / Rocky / AlmaLinux** — XFS by default → no `reserved_bytes` (`null`); use `df -i` for inodes; LVM common (`vgs`/`lvs`/`pvs --units b`). `/boot/efi` is present only on UEFI x86_64.
- **Debian / Ubuntu** — ext4 by default → `reserved_bytes` via `tune2fs -l` (reserved blocks × block size); LVM optional.
- **SUSE / SLES** — btrfs by default (subvolumes + snapshots) → `df` is misleading; use `btrfs filesystem usage` / qgroups for the real byte figures; LVM optional.
